...
function cumlelereDonustur(cumleler, sep = ",") {
  const yeniCumle = cumleler.map((kelimeler) => kelimeler.join(sep));
  return yeniCumle;
}
/*
function cumlelereDonustur(cumleler, sep = ",") {
  const yeniCumle = cumleler.map(function (kelimeler) {
    return kelimeler.join(sep);
  });
  return yeniCumle;
}
*/
...
function paragrafOlustur(cumleler, cumleFn, donusturFn) {
  const paragraf = donusturFn(cumleler, " ");
  const birlesikCumle = cumleFn(
    paragraf[1],
    paragraf[3],
    paragraf[5],
    paragraf[7],
...
function emojileriDonustur(mesaj, emoji) {
  for (var sembol in emoji) {
    var buyukHarfli = sembol.toUpperCase();
    var kucukHarfli = sembol.toLowerCase();

    mesaj = mesaj.replaceAll(sembol, emoji[sembol]);
    mesaj = mesaj.replaceAll(buyukHarfli, emoji[sembol]);
    mesaj = mesaj.replaceAll(kucukHarfli, emoji[sembol]);
  }
  return mesaj;
}
